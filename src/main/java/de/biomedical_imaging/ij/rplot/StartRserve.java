	...
	public static boolean launchRserve(String cmd, String rargs, String rsrvargs, boolean debug) {
		...
		try {
			...
			if (!isWindows) /* on Windows the process will never return, so we cannot wait */
				p.waitFor();
		//	IJ.log("call terminated, let us try to connect ...");
		} catch (Exception x) {
			System.out.println("failed to start Rserve process with "+x.getMessage());
			IJ.log("failed to start Rserve process with "+x.getMessage());
			IJ.log("Stack Trace"+x.getStackTrace());
			return false;
		}
	...
	public static boolean checkLocalRserve() {
		...
		if (osname != null && osname.length() >= 7 && osname.substring(0,7).equals("Windows")) {
			...
			try {
				...
				IJ.log("ERROR: unable to run REG to find the location of R: "+rge);
				return false;
			}
			if (installPath == null) {
				System.out.println("ERROR: canot find path to R. Make sure reg is available and R was installed with registry settings.");
				IJ.log("ERROR: canot find path to R. Make sure reg is available and R was installed with registry settings.");
				return false;
			}
